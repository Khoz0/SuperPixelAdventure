...
Game* createGame(){
  ...
  createBar(getGameAtlas(game), getScreen(getGameSdl(game)));
...
void runGame(Game* game) {
    ...
    while (!getVariable(getGameVariables(game), GAMEOVER)) {
      ...
      // updating the position of the hero (xchar, ychar)
      updatePosChar(game);
      ...
      hitBars(game);
      cpt += 1;
      updatePositionNeg(game, cpt, animation);

      // all the prints (map, hero, PNJ, pannel, ...)
      display(game);
...
void updatePositionNeg(Game* game, int cpt, int animation) {
  if (cpt % 3 == 0){
    if (animation){
      animation = 0;
    }else{
